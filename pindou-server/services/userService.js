...
/**
 * 找回密码（无短信依赖）：使用部署时配置的站内私人口令 PASSWORD_RESET_CODE 校验。
 * 适合未接入短信的小型/私用部署；正式对外建议换成短信或密保。
 */
async function resetPassword(phone, code, newPassword) {
  const p = assertPhone(phone)
  const inputCode = String(code ?? '').trim()
  const np = String(newPassword ?? '')
  if (!config.passwordResetCode) throw new HttpError(400, '未配置找回口令，请联系站长')
  if (!inputCode) throw new HttpError(400, '请输入找回口令')
  if (inputCode !== config.passwordResetCode) throw new HttpError(400, '找回口令错误')
  if (np.length < 6) throw new HttpError(400, '新密码至少 6 位')

  const [rows] = await pool.query('SELECT id, password FROM users WHERE username = ?', [p])
